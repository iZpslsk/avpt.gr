...
/**
 * данные путевых объектах асим
 */
public class Block32_C4_0 {
    ...
    public Block32_C4_0(byte[] values) {
        this.values = values;
    ...
    private int getAmountObj() {
        return 1;//toUnsignedInt(values[0]) & 0x1F;
    }
    ...
    private void fillArrId() {
        int index = 0; // индекс первого путевого объекта
        for (int i = 0; i < arrId.length; i++) {
            if (index >= 0 && index < values.length) {
                arrId[i] = toUnsignedInt(values[index]);
